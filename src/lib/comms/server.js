...
module.exports = function(container) {
    ...
    if (container.settings.expressAuth === 1) {
        ...
        var basic = auth.basic({
            file: __dirname + path.join(process.cwd(), container.settings.expressAuthFile)
        });
    ...
    app.get('/pumpCommand/run/pump/:pump/program/:program', function(req, res) {
        ...
        response.pump = pump
        response.program = program
        container.pumpControllerMiddleware.pumpCommandRunProgram(pump, program)
        res.send(response)
    })
    ...
    app.get('/pumpCommand/run/pump/:pump/rpm/:rpm', function(req,res){
      ...
      response.pump = pump
      response.speed = rpm
      container.pumpControllerMiddleware.pumpCommandRunRPM(pump, rpm)
      res.send(response)
    })
    ...
    app.get('/pumpCommand/run/pump/:pump/program/:program/duration/:duration', function(req, res) {
        ...
        container.pumpControllerMiddleware.pumpCommandRunProgramForDuration(pump, program, duration)
        res.send(response)
    })
    ...
    app.get('/pumpCommand/run/pump/:pump/rpm/:rpm/duration/:duration', function(req, res) {
        ...
        response.duration = duration
        container.pumpControllerMiddleware.pumpCommandRunRPMForDuration(pump, rpm, duration)
        res.send(response)
    })
