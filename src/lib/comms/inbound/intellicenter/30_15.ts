...
// if (logModuleLoading)
//   logger.info('Loading: intellicenter_30_15.js')
...
export function process ( data: number[], counter: number )
{
  ...
  if ( settings.get( 'logMessageDecoding' ) )
    logger.debug( `Msg#: ${ counter }:  ${ constants.intellicenterPackets[ data[ subAction ] ] }   \n\tFull packet: $s`, data );
...
/*istanbul ignore next */
    // if (logModuleLoading)
    //   logger.info('Loaded: intellicenter_30_15˝.js')
