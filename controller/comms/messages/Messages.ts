...
export class Message
{
    ...
    protected _headerSubByte: number = 0;
    public static pluginAddress: number = config.getSection( 'controller', { address: 33 } ).address;
    public controllerType: ControllerType = ControllerType.IntelliCenter
    // Fields
    ...
    public get headerSubByte (): number { return this._headerSubByte || 33 }
    public set headerSubByte ( val: number ) { this._headerSubByte = val; }
    ...
    public toLog (): string
    {
        //let tzo = this.timestamp.getTimezoneOffset();
        //new Date(this.timestamp.getTime() - (tzo * 60000)).toISOString().slice(0, -1) + (tzo > 0 ? '-' : '+') + pad(tzo / 60) + pad(tzo % 60)
        //var pad = function (n) {
    ...
    private ( val: number ) { if ( this.protocol !== Protocol.Chlorinator ) this.header[ 2 ] = val; }
}
export class Inbound extends Message
{
    ...
    public readPacket ( bytes: number[] ): number
    {
        ...
        ndx = this.readPayload( bytes, ndx );
...
export class Response extends Message
{
    constructor( source: number, dest: number, action?: number, payload?: number[], ack?: number, callback?: ( msg?: Outbound ) => void )
    {
        ...
        this.direction = Direction.In;
        this.preamble.push.apply( this.preamble, [ 255, 0, 255 ] );
        this.header.push.apply( this.header, [ 165, super.headerSubByte, 0, 0, 0, 0 ] );
        this.term.push.apply( this.term, [ 0, 0 ] );
        this.source = source;
        this.dest = dest;
        this.action = action;
