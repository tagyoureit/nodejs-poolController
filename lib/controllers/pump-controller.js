    ...
    function pumpCommandSaveSpeed(index, program, speed) {
        ...
        if (pump > -1 && program >= 1 && program <= 4) {
            ...
            if (speed <= 450 || speed >= 3450 || isNaN(speed) || speed == null) {
                ...
                if (container.settings.logApi) logger.verbose('User request to save pump %s (address %s) to Program %s as %s RPM', equip, pump, program, speed);
        ...
        logger.warn('FAIL: User request to save pump %s (address %s) to Program %s as %s RPM', equip, pump, program, speed);
        return false
    }
