...
import { settings, logger, chlorinator, temperature, UOM, io } from'../../etc/internal';
...
export namespace intellichem
{
    ...
    {
        var TF = 0;
        var temp = temperature.getTemperature().poolTemp
        if ( UOM.getUOMStr() === 'Fahrenheit' )
        {
            if ( temp <= 32 ) TF = 0.0
    ...
    export function correctedAlkalinity ()
    {
        return intellichem.settings.TOTALALKALINITY - ( intellichem.settings.CYA / 3 )
    }

    export function calculateTotalCarbonateAlkalinity ()
    {
    ...
    export function calculateTotalDisolvedSolidsFactor ()
    {
        // 12.1 for non-salt pools; 12.2 for salt pools
        return settings.get( 'chlorinator' ).installed ? 12.2 : 12.1
    }

    export function processIntellichemControllerPacket ( data: number[], counter: number )
    {

        intellichem.readings.PH = ( ( data[ constants.intellichemPacketFields.PHREADINGHI ] * 256 ) + data[ constants.intellichemPacketFields.PHREADINGLO ] ) / 100
        intellichem.readings.ORP = ( data[ constants.intellichemPacketFields.ORPREADINGHI ] * 256 ) + data[ constants.intellichemPacketFields.ORPREADINGLO ]
        intellichem.readings.WATERFLOW = data[ constants.intellichemPacketFields.WATERFLOW ]
        intellichem.readings.SALT = settings.get( 'chlorinator' ) ? chlorinator.getSaltPPM() : 0

        intellichem.settings.PH = ( ( data[ constants.intellichemPacketFields.PHSETPOINTHI ] * 256 ) + data[ constants.intellichemPacketFields.PHSETPOINTLO ] ) / 100
        intellichem.settings.ORP = ( data[ constants.intellichemPacketFields.ORPSETPOINTHI ] * 256 ) + data[ constants.intellichemPacketFields.ORPSETPOINTLO ]
        intellichem.settings.CYA = data[ constants.intellichemPacketFields.CYAREADING ]
        intellichem.settings.CALCIUMHARDNESS = ( data[ constants.intellichemPacketFields.CALCIUMHARDNESSHI ] * 256 ) + data[ constants.intellichemPacketFields.CALCIUMHARDNESSLO ]
        intellichem.settings.TOTALALKALINITY = ( data[ constants.intellichemPacketFields.TOTALALKALINITYREADINGHI ] * 256 ) + data[ constants.intellichemPacketFields.TOTALALKALINITYREADINGLO ]

        intellichem.tankLevels[ 1 ] = data[ constants.intellichemPacketFields.TANK1 ]
        intellichem.tankLevels[ 2 ] = data[ constants.intellichemPacketFields.TANK2 ]

        intellichem.mode[ 1 ] = data[ constants.intellichemPacketFields.MODE1 ]
        intellichem.mode[ 2 ] = data[ constants.intellichemPacketFields.MODE2 ]

        if ( !_.isEqual( intellichem.lastPacket, data ) )
        {
            intellichem.lastPacket = _.clone( data )
            ...
            if ( settings.get( 'logIntellichem' ) )
            {
                logger.info( 'Msg# %s  Intellichem packet found: \n\t', counter, JSON.stringify( _.omit( intellichem, 'lastPacket' ), null, 2 ) )
                logger.debug( 'Msg# %s  Intellichem packet: %s', counter, data )
                logger.info( 'Msg# %s  Intellichem Saturation Index:\n\tSI = pH + CHF + AF + TF - TDSF\n\t%s = %s + %s + %s + %s - %s', counter, intellichem.readings.SI, intellichem.readings.PH, calculateCalciumHardnessFactor(), calculateTotalCarbonateAlkalinity(), calculateTemperatureFactor(), calculateTotalDisolvedSolidsFactor() )
            }
            io.emitToClients( 'intellichem', { intellichem: intellichem } )
        }
        else
        {
            logger.debug( 'Msg# %s  Duplicate Intellichem packet.' )
        }

    }

    export function getCurrentIntellichem ()
    {
