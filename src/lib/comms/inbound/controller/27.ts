...
import * as c from '../../../../etc/constants'
  ...
  // if (logModuleLoading)
  //   logger.info('Loading: 27.js')

  var pumpConfig: any = {}
var pumpNum: number;;

  export function associate_circuit(circuitnum:number) {
    var _circuit: any = {}
    if (circuitnum === 0) {
      _circuit.number = 0
      _circuit.name = 'none'
      _circuit.friendlyName = 'none'
    } else if (circuitnum <= circuit.getNumberOfCircuits()) {
      _circuit.number = circuitnum
      _circuit.name = circuit.getCircuitName(circuitnum)
      _circuit.friendlyName = circuit.getFriendlyName(circuitnum)
    } else {
      _circuit.number = circuitnum
      _circuit.name = _circuit.friendlyName = c.strCircuitFunction[circuitnum]
    }
    return _circuit
  }

  export function rpmOrGPM(high: number, low: number, flag: number, circuit_slot: { number: number; flag: string; gpm: any; rpm: any; }) {
  ...
  function parseVSPump(data: number[]) {
  ...
  function parseVSFPump(data:  number[]) {
  ...
  function parseVFPump(data: any[] | number[]) {
...
export function getExtendedPumpConfig ()
{
    return pumpConfig
  }
  ...
  export function process(data: number[], counter: number) {
    ...
    logger.info("Discovered pump configurations: %s", JSON.stringify(pumpConfig, null, 2))
