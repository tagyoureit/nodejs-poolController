...
export class PumpMessage
{
    ...
    {
        // packet 24/27 - Pump Config: IntelliTouch
        var pumpId = msg.extractPayloadByte( 0 );
        var pump: Pump = sys.pumps.getItemById( pumpId, pumpId !== 0 );
        if ( pumpId > sys.equipment.maxPumps ) sys.equipment.maxPumps = pumpId;
    ...
    private static processIntelliCenterPump ( msg: Inbound )
    {
        ...
        if ( msg.extractPayloadByte( 1 ) <= 15 )
        {
            ...
            else
            {
                ...
                {
                    if ( msg.extractPayloadByte( i ) !== 255 ) pump.circuits.add( { id: circuitId, circuit: msg.extractPayloadByte( i ) + 1 } );
                    circuitId++;
                }
            }
            // Speed/Flow 
    ...
    private static processFlowStepSize ( msg: Inbound )
    {
        ...
        {
            var pump: Pump = sys.pumps.getItemById( pumpId++ );
    ...
    private static processPumpType ( msg: Inbound )
    {
        ...
        for ( let i = 2; i < msg.payload.length && pumpId <= sys.equipment.maxPumps; i++ )
        {
            var pump: Pump = sys.pumps.getItemById( pumpId++, msg.extractPayloadByte( i ) !== 0 );
    ...
    {
        var pumpId = ( ( msg.extractPayloadByte( 1 ) - 19 ) * 2 ) + 1;
    ...
    private static processVS_IT ( msg: Inbound )
    {
        ...
        let pumpId = msg.extractPayloadByte( 0 );
        let pump = sys.pumps.getItemById( pumpId );
        for ( let circuitId = 1; circuitId <= this.maxCircuits; circuitId++ )
        {
            ...
            let circuit: PumpCircuit = pump.circuits.getItemById( circuitId );
            circuit.speed = msg.extractPayloadByte( ( circuitId * 2 ) + 4 ) * 256 + msg.extractPayloadByte( circuitId + 21 );
            circuit.units = 0;
        }
    ...
    private static processVSF_IT ( msg: Inbound )
    {
        ...
        for ( let circuitId = 1; circuitId <= this.maxCircuits; circuitId++ )
        {
            if ( msg.extractPayloadByte( ( circuitId * 2 ) + 3 ) !== 0 ) pump.circuits.add( { id: circuitId, circuit: msg.extractPayloadByte( ( circuitId * 2 ) + 3 ) } );
            let circuit: PumpCircuit = pump.circuits.getItemById( circuitId );
            let flag = (msg.extractPayloadByte(4) & circuitId) >> (circuitId - 1) === 0    
            if ( flag )
            {
            ...
            {
                circuit.speed = ( msg.extractPayloadByte( ( circuitId * 2 ) + 3 ) * 256 ) + msg.extractPayloadByte( circuitId + 21 );
                circuit.units = 0;
                pump.minSpeed = 450;
                pump.maxSpeed = 3450;
                pump.speedStepSize = 100;
