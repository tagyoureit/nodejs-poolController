...
export function process ( data: number[], counter: number )
{
  ...
  if ( ( data[ 6 ] >> 7 ) !== pumpOffDuringValveOperation || pumpOffDuringValveOperation === -1 )
  {
    ...
    if ( settings.get( 'logMessageDecoding' ) )
      logger.debug( 'Msg#: %s  Delay Status packet. pumpOffDuringValveOperation: %s  Full packet: %s', counter, pumpOffDuringValveOperation === 1 ? 'on' : 'off', data );
  } else
  {
    if ( settings.get( 'logMessageDecoding' ) )
      logger.debug( 'Msg#: %s  Delay Status packet, but something unknown changed.  Full packet: %s', counter, data );
